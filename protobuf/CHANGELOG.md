...
* **Breaking change:** these names are no longer reserved: `fromBuffer`,
  `fromJson`, `$_defaultFor`, `initByValue`. If you have proto fields with
  these names, your fields will not have a suffix in generated Dart code. E.g.
  if you had `from_buffer` field in a message previously in the generated Dart
  it would be renamed to something like `fromBuffer_1`. Now it will just be
  `fromBuffer`, without any suffix. ([#618], [#634])
